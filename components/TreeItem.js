...
export default class TreeItem extends HTMLElement {
  constructor() {
    ...
    this.shadowRoot.innerHTML = `
      <li>
        <div class="${tw`w-full py-1 hover:bg-gray-100 rounded-lg`}">
          <div id="head" class="${tw`flex items-center`}">
            <div id="expand-icon" class="${tw`px-1 invisible`}"></div>
            <slot name="icon"></slot>
            <slot name="body"></slot>
          </div>
        </div>
  ...
  appendChild(child) {
    ...
    expandIcon.classList.remove(tw('invisible'));
  }
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if (name === 'nest') {
      ...
      head.classList.add(tw(`ml-${Number(newValue) * 2}`));
    } else if (name === 'expand') {
      const expandIcon = this.shadowRoot.querySelector('#expand-icon');
      const children = this.shadowRoot.querySelector('#children');
      if (newValue === 'true') {
