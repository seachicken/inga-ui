...
document.querySelector('#app').innerHTML = `
  ...
  <div class="${tw`flex h-screen`}">
    <div id="entorypoint-nav" class="${tw`w-72 pt-2`}">
      <div class="${tw`flex items-center w-full`}">
        ...
        <span class="${tw`ml-2 text-2xl`}">Files affected</span>
      </div>
...
entorypointTreeView.addEventListener('itemselect', (e) => {
  ...
  function findIndex(target) {
    for (let fi = 0; fi < originTree.length; fi += 1) {
      const file = originTree[fi];
      if (file.type !== fileType.FILE) {
        continue;
      }
      for (let di = 0; di < file.declarations.length; di += 1) {
        const originDec = file.declarations[di];
        if (originDec.origins.some((o) => o.declarations.some(
          (d) => equalsDeclaration(d, target),
        ))) {
          return { fileIndex: fi, declarationIndex: di };
        }
      }
    }
    return false;
  ...
  for (const entoryOrigin of selectedEntoryDec.origins) {
    for (const entoryDec of entoryOrigin.declarations) {
      const found = findIndex(entoryDec);
      if (found) {
        indexes.push(found);
      }
    }
  ...
  originTreeView.relatedindexes = JSON.stringify(indexes);
});
