...
function reload(poss) {
  ...
  document.querySelector('#app').innerHTML = `
    <div class="absolute flex w-full h-full">
      <button id="refresh-button" class="flex items-center fixed mt-2 z-10 inset-x-0 max-w-max mx-auto bg-blue-500 text-white rounded-full px-3 py-1 hidden">
      ...
      <service-graph id="service-graph" class="relative w-full h-full overflow-auto bg-gray-100" src=${JSON.stringify(graphs)} repourl="${repoUrl}" prnumber="${prNumber}"></service-graph>
    </div>
  `;
